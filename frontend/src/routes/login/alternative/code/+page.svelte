...
<SignInWrapper isWide={isLongCode}>
	...
	>
		<div class="flex flex-col w-full justify-center {data.clientIdPresent ? 'pt-6' : ''}">
			{#if !data.clientIdPresent}
					...
					{#snippet child({ props })}
						<button type="button" onclick={() => (isLongCode = true)} {...props}>
							<Item.Content class="w-full items-center text-center">
								<Item.Title class="text-center">I have a longer code</Item.Title>
							</Item.Content>
						</button>
					{/snippet}
				</Item.Root>
